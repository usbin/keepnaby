...
파라미터: [모터번호, 위치값]
모터 0 = 시침, 모터 1 = 분침
위치값 = 0~59 (다이얼 1분 단위 눈금 기준)
```
**시 마크 매핑:** 시계 1시~12시 라벨 위치는 0–59 스케일에서 `시 × 5 % 60`.
- 1시 → 위치 5
- 2시 → 위치 10
- ...
- 11시 → 위치 55
...
**recalibrate 모드에서만 유효.** recalibrate(false)로 빠져나오면 `datetime` 명령으로 시각 재설정 필수.

**양바늘 동시 이동 불가:** `stepper_goto`는 단일 모터 전용. 두 바늘을 같은 위치로 보내려면 [0, X]/[1, X]를 연속 발행해야 하며, iOS CoreBluetooth의 `.withResponse` ACK 간격(~20–30ms)만큼 미세한 도착 시차가 생김.

**시각모드 자동 복귀:** recalibrate 모드에서도 `stepper_goto`만으로 바늘을 옮긴 뒤 일정 시간 동안 추가 명령이 없으면 시계가 시각모드로 돌아가려는 동작이 관찰됨. 앱이 지속적으로 바늘을 제어하는 경로(예: 모스부호 입력 피드백)에서는 `recalibrate_move`(상대 이동) 사용을 권장.
...
#### `recalibrate_move` — 캘리브레이션 중 바늘 이동 (상대)
```
파라미터: [모터번호, 스텝수]
모터 0 = 시침, 모터 1 = 분침
스텝수 = ±정수 (양수=시계방향, 음수=반시계방향)
```
**스케일 (Kronaby Nord 실측):**
- 분침(motor 1): 3 step / 1분 마크
- 시침(motor 0): 2 step / 1분 마크 (15 step → 7.5분)
- 두 모터의 기어비가 다르므로 같은 마크 수 이동 시 다른 step 값 필요

**양 모터 모두 음수 delta 지원** — 반시계 회전. shortest path로 직진 이동 가능 (forward wrap 불필요).

**상대 이동:** `stepper_goto`(절대)와 달리 현재 위치에서 ±스텝수만큼 이동. 앱에서 절대 위치를 추적하면 절대 이동도 구현 가능.
...
| 11 | 버튼 매핑 | 상단 5조합 + 하단 4조합 + 모스부호 입력 모드 + 날짜/배터리/걸음수 표시 | ✅ |
...
| 24 | 모스부호 입력 모드 | 하단 길게→진입, 점/대시 입력→상단 1회 확정, 하단 길게→명령 실행 | ✅ |
| 25 | 모스 모드 바늘 피드백 | 분침 누적(점=+5, 대시=+10), 시침 N×5(글자수), recalibrate_move 기반 | ✅ |
...
- **하단 버튼**: 1회/2회/3회/4회 클릭 = **4개 조합** (길게 = 모스부호 모드 진입/종료 고정)
- **크라운**: BLE 이벤트 미전송. complications 전용
- 할당 가능 액션: 폰 찾기, 오늘 날짜 확인, 배터리 잔량 표시, 걸음수 확인, 음악 제어, 위치 기록, 랜덤 주사위, IFTTT Webhook, iOS 단축어, URL 요청

### 모스부호 입력 모드
- **진입**: 하단 길게 → 진동 1회 + `recalibrate(true)`로 양 바늘 12시(0) 정렬
- **입력 제스처:**
  - 하단 1회 = 점(`.`), 하단 2회 = 대시(`-`)
  - 상단 1회 = 한 문자 확정 (현재 심볼 버퍼 → 누적 명령어 버퍼)
  - 상단 2회 = 점진 취소 (심볼 버퍼 → 1자 → 빈 상태 진동)
  - 상단 길게 = 전체 취소 (두 버퍼 비우고 양 바늘 0 복귀)
  - 하단 길게 = 종료 (확정된 명령어 실행, 미등록이면 진동 5회)
- **바늘 피드백** (모드 진입 시 시작된 `recalibrate(true)` 세션 동안):
  - 분침: 매 심볼마다 점=+5분, 대시=+10분 누적 (mod 60), 글자 사이 0 복귀 없음
  - 시침: 글자 확정 시 `count × 5분` 위치로 직접 이동 (1자=5분 ... 11자=55분)
  - 모든 이동은 `recalibrate_move(motor, ±delta)` 음수 delta 직진 사용
- **명령어 제약**: 영숫자 대문자, 최대 11자 (시침 카운터 0~55분, 12시는 안 가리킴)
- **종료 시**: 양 바늘 0 복귀 → `recalibrate(false)` → `datetime` 재전송으로 시각 동기화
