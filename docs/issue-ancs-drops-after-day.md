...
| 시도 | 결과 |
|------|------|
| 6시간 주기 `cancelPeripheralConnection` 강제 재연결 | ANCS 복구 안 됨 (iOS BLE 스택 리셋이 아니라 단일 peripheral disconnect라서) |
| `BGAppRefreshTask`로 백그라운드에서 강제 재연결 | 위와 같은 이유로 무의미 |
| 10분 keepAlive에서 `complications`/`ancs_filter` 반복 재전송 | 효과 미확인. 공식 앱은 이걸 안 하므로 오히려 원인일 가능성 |
...
- **현재 조치: keepAlive에서 ANCS 명령 재전송 제거 (v1.0.5에서 테스트 예정)**
...
## 아직 시도해볼 것
1. **keepAlive ANCS 재전송 제거 빌드 테스트** — 가장 유력한 가설, 하루 테스트로 확인 가능
